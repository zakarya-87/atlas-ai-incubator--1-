...
export class VenturesService {
  ...
  async inviteMember(ventureId: string, ownerId: string, email: string, role: string) {
    ...
    if (venture.userId !== ownerId) throw new ForbiddenException('Only the owner can invite members');

    // 2. Find User to Invite
    const invitee = await this.usersService.findOne(email);
    if (!invitee) throw new NotFoundException('User with this email not found. They must register first.');

    if (invitee.id === ownerId) throw new BadRequestException('You cannot invite yourself');
    ...
    this.emailService.sendInviteEmail(email, ownerName, venture.name).catch(console.error);
  ...
  async checkAccess(ventureId: string, userId: string): Promise<boolean> {
      const venture = await (this.prisma as any).venture.findUnique({ where: { id: ventureId } });
