...
export class ReportsService {
  ...
  async generatePDFReport(analysisId: string, userId?: string): Promise<Buffer> {
    if (this.reportCache.has(analysisId) && !userId) { // Simple cache for testing
    ...
    const htmlContent = this.buildHtml(analysis.tool, data, analysis.inputContext || analysis.inputDescription);
    ...
    try {
        ...
        this.reportCache.set(analysisId, { ...analysis, pdf: Buffer.from(pdfBuffer) });
  ...
  async generateHTMLReport(analysisId: string, userId?: string): Promise<string> {
    const analysis = await (this.prisma as any).analysis.findUnique({
        where: { id: analysisId },
        include: { venture: true }
    });
    ...
    if (userId && analysis.venture.userId !== userId) throw new ForbiddenException('Access denied');

    const content = this.buildHtml(analysis.tool, analysis.resultData, analysis.inputContext || analysis.inputDescription);
  ...
  async generateCustomReport(analysisId: string, template: any, userId?: string): Promise<string> {
    const analysis = await (this.prisma as any).analysis.findUnique({
        where: { id: analysisId },
        include: { venture: true }
    });
  ...
  async batchGeneratePDFs(analysisIds: string[], userId?: string): Promise<any[]> {
    const results = [];
