...
export const exportToPdf = async (
  ...
  if (!element) {
    console.error(
      `Element with id "${targetElementId}" not found for PDF export.`
    );
    return;
  }
  ...
  try {
    ...
    pdf.text(businessDescription, pdfWidth / 2, margin + 20, {
      align: 'center',
...
const convertToCsv = (data: AnalysisData, type: AnalysisType): string => {
  ...
  const headers: Record<AnalysisType, string> = {
    ...
    budgetGenerator:
      'Scenario,Category,Item Name,Item Type,Amount (12-Month Total)\n',
    ...
    pitchDeck:
      'Slide Number,Slide Title,Point,Explanation,Visual Suggestion Type,Visual Suggestion Description\n',
    investorDatabase:
      'Investor Name,Type,Location,Stage,Ticket Size,Sector Focus,Alignment Score,Reasoning,Outreach Subject,Outreach Body\n',
  ...
  } else if (type === 'roadmap' && 'phases' in data && 'milestones' in (data.phases[0] || {})) {
