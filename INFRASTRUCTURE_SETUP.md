...
_Verify: Run `docker ps` to ensure the container `atlas-backend-postgres-1` is running on port 5432._
...
_Note: We use `db push` for rapid prototyping. For production, we will use `prisma migrate`._
