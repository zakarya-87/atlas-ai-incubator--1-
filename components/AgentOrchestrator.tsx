...
const AgentOrchestrator: React.FC<AgentOrchestratorProps> = ({
  ...
  const getLogsForToolSimulated = (
    ...
    if (['swot', 'pestel'].includes(tool)) {
      baseLogs.push({
        ...
        messageKey: 'agentLogScanningMarket',
      });
      baseLogs.push({
        agent: 'Lead Strategist',
        messageKey: 'agentLogIdentifyingFactors',
      });
      baseLogs.push({
        agent: 'Lead Strategist',
        messageKey: 'agentLogSynthesizing',
      });
    } else if (['financialForecast', 'budgetGenerator'].includes(tool)) {
      baseLogs.push({
        agent: 'CFO',
        messageKey: 'agentLogReviewingBenchmarks',
      });
      baseLogs.push({
        agent: 'CFO',
        messageKey: 'agentLogCalculatingProjections',
      });
      baseLogs.push({ agent: 'CFO', messageKey: 'agentLogOptimizingMargins' });
    } else if (['pitchDeckGenerator', 'investorDatabase'].includes(tool)) {
      baseLogs.push({
        agent: 'Venture Capitalist',
        messageKey: 'agentLogEvaluatingThesis',
      });
      baseLogs.push({
        agent: 'CFO',
        messageKey: 'agentLogCheckingUnitEconomics',
      });
      baseLogs.push({
        agent: 'Venture Capitalist',
        messageKey: 'agentLogDraftingNarrative',
      });
    } else {
      baseLogs.push({ agent: mainAgent, messageKey: 'agentLogProcessingData' });
      baseLogs.push({
        agent: mainAgent,
        messageKey: 'agentLogGeneratingInsights',
      });
    }
  ...
  const agentAvatars: Record<AgentType, React.ReactNode> = {
    ...
    'Market Researcher': (
      <div className="bg-blue-500 h-full w-full flex items-center justify-center text-white font-bold">
        MR
      </div>
    ),
    CFO: (
      <div className="bg-green-600 h-full w-full flex items-center justify-center text-white font-bold">
        CF
      </div>
    ),
    'Growth Hacker': (
      <div className="bg-red-500 h-full w-full flex items-center justify-center text-white font-bold">
        GH
      </div>
    ),
    'Product Owner': (
      <div className="bg-orange-500 h-full w-full flex items-center justify-center text-white font-bold">
        PO
      </div>
    ),
    'Venture Capitalist': (
      <div className="bg-yellow-500 h-full w-full flex items-center justify-center text-white font-bold">
        VC
      </div>
    ),
    'Systems Architect': (
      <div className="bg-slate-500 h-full w-full flex items-center justify-center text-white font-bold">
        SA
      </div>
    ),
    'Creative Director': (
      <div className="bg-pink-500 h-full w-full flex items-center justify-center text-white font-bold">
        CD
      </div>
    ),
  };
    ...
    <div data-testid="agent-boardroom" className="w-full max-w-7xl mx-auto mt-8 space-y-8 animate-fade-in">
          ...
          <div className="space-y-4">
            {getAgentsForTool(activeTool).map((agent) => (
              ...
              >
                ...
                <div>
                  <p
                    className={`text-xs font-bold ${activeAgent === agent ? 'text-brand-teal' : 'text-brand-text'}`}
                  >
                    {t(agent.replace(' ', '') as TranslationKey)}
                  </p>
                  {activeAgent === agent && (
                    <div className="flex items-center mt-0.5">
                      ...
                      <span className="text-[10px] text-brand-light uppercase">
                        Active
                      </span>
                    </div>
                  )}
          ...
          <div className="flex-grow overflow-y-auto space-y-2 pr-2 custom-scrollbar">
            <AnimatePresence>
              {logs.map((log) => (
                ...
                >
                  <span className="text-brand-light/40 mr-2 rtl:ml-2 rtl:mr-0 flex-shrink-0 font-light">
                    [
                  ...
                  <div>
                    <span className="font-bold text-brand-teal mr-1 rtl:ml-1 rtl:mr-0">
                      @{t(log.agent.replace(' ', '') as TranslationKey)}:
                    </span>
                    <span className="text-brand-text/90">
                      {t(log.messageKey as TranslationKey)}
                    </span>
                  </div>
                </motion.div>
              ))}
      ...
      <div className="w-full opacity-70 transition-opacity duration-1000 pointer-events-none">
        <div className="flex items-center gap-3 mb-6 px-2 border-b border-brand-accent/20 pb-2 max-w-7xl mx-auto">
          ...
          <span className="text-xs font-mono text-brand-teal uppercase tracking-[0.2em]">
            Drafting Workspace
          </span>
        </div>
