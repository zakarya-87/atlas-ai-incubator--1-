...
const Header: React.FC<HeaderProps> = memo(({ onNavigate, onStartTour, onShareClick, onToggleSidebar }) => {
  const { t } = useLanguage();
  const { user, isAuthenticated, isAdmin, logout, signInAsAdminDemo, loading } = useAuth();
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <div className="md:hidden">
              <button
                type="button"
                className="p-2 -ml-2 rounded-md text-brand-light hover:text-white hover:bg-white/10"
            ...
            <div className="flex-shrink-0 flex items-center gap-2">
              <img src="/logo.svg" alt={t('headerLogoAlt')} className="h-8 w-8" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
              <h1 className="text-xl font-bold text-brand-text tracking-tight hidden sm:block">
                {t('headerTitle')}
              </h1>
            </div>
            ...
            <div id="language-switcher" className="flex items-center gap-2">
              ...
              {loading ? (
                <div className="text-sm text-brand-light px-4">{t('headerLoading')}</div>
              ) : isAuthenticated ? (
                <div className="flex items-center gap-4 border-l border-white/10 pl-4 ml-2">
                  ...
                  <Tooltip content={isPro ? "Unlimited Access" : "Remaining AI Generations"} position="bottom">
                    <div className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 border ${isPro
