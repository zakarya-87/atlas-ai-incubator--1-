...
const SkeletonBlock: React.FC<SkeletonBlockProps> = ({ className = "", rows = 3 }) => (
  <div className={`bg-brand-secondary/20 rounded-lg p-4 animate-pulse border border-white/5 ${className}`}>
    <div className="h-5 w-1/3 bg-brand-light/10 rounded mb-4"></div>
    {Array.from({ length: rows }).map((_, i) => (
      <div key={i} className={`h-3 bg-brand-light/5 rounded mb-2 ${i === rows - 1 ? 'w-3/4' : 'w-full'}`}></div>
    ))}
  </div>
);
...
const PitchDeckSkeleton = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[1,2,3,4].map(i => <SkeletonBlock key={i} className="h-64" rows={5} />)}
    </div>
);
