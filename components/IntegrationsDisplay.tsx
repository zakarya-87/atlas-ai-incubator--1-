...
const ConnectorCard: React.FC<ConnectorCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`bg-brand-secondary/40 p-6 rounded-lg flex flex-col md:flex-row items-center gap-6 border transition-colors ${isConnected ? 'border-green-500/30 bg-green-900/5' : 'border-brand-accent/50'}`}
    >
...
const IntegrationsDisplay: React.FC<IntegrationsDisplayProps> = ({
  ventureId,
}) => {
  const { t } = useLanguage();
  ...
  const handleToggle = async (providerId: string) => {
    try {
      const isCurrentlyConnected =
        integrations.find((i) => i.provider === providerId)?.status ===
        'connected';
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="text-center mb-8">
        ...
        <p className="mt-2 text-brand-light max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        {connectors.map((connector) => (
          <ConnectorCard
            ...
            {...connector}
            isConnected={
              integrations.find((i) => i.provider === connector.id)?.status ===
              'connected'
            }
