...
const SwotQuadrant: React.FC<SwotQuadrantProps> = ({ title, points, icon, bgColorClass, textColorClass, category, fullData, onUpdate }) => {
  ...
  const handleSaveEdit = () => {
    ...
    const updatedPoints = [...points];
    updatedPoints[editingIndex] = { ...updatedPoints[editingIndex], point: editedPoint.trim(), explanation: editedExplanation.trim() };
    ...
    >
      ...
      <ul className="space-y-3 text-brand-text/90 flex-grow">
        {points.map((item, index) => (
          <li key={index} className="text-sm leading-relaxed group relative p-2 -m-2 rounded-md hover:bg-white/5 transition-colors grid">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                >
                    ...
                    {item.completed && (
                        <motion.svg
                            ...
                            className="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}
                        >
                ...
                <div className={`flex-grow transition-all duration-300 ${item.completed ? 'opacity-60' : ''}`}>
                    <strong className={`font-semibold text-brand-text/95 display-block transition-all ${item.completed ? 'line-through' : ''}`}>{item.point}</strong>
                    <p className={`text-brand-text/80 transition-all ${item.completed ? 'line-through' : ''}`}>{item.explanation}</p>
                </div>
              </div>
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L13.5 6.5z" /></svg>
              </button>
            </motion.div>
              ...
              {editingIndex === index && (
                ...
                >
                  ...
                  <div className="flex items-center justify-end space-x-2">
                    <button onClick={handleCancelEdit} className="px-3 py-1 text-xs text-brand-light hover:text-white rounded-md hover:bg-brand-accent/50 transition-colors">Cancel</button>
                    <button onClick={handleSaveEdit} className="px-3 py-1 text-xs bg-brand-teal rounded-md text-white font-semibold hover:bg-teal-500 transition-colors">Save</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
...
const icons = {
  ...
  weaknesses: (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.738 3h4.017c.163 0 .326.02.485.06L17 4m-7 10v5a2 2 0 002 2h.085a2 2 0 001.736-.93l2.5-4.5M17 14h-2.5" />
    </svg>
  ),
  opportunities: (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
