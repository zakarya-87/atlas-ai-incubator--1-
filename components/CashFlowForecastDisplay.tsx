  ...
  <div className="bg-brand-secondary/50 p-4 rounded-lg flex items-center">
    ...
    <div>
      ...
      <p className="text-2xl font-bold text-brand-text">
        {value}{' '}
        <span className="text-lg font-medium text-brand-light">{subtext}</span>
      </p>
    </div>
  </div>
);

const CashFlowForecastDisplay: React.FC<{ data: CashFlowForecastData }> = ({
  data,
}) => {
                ...
                <thead className="text-xs text-brand-text uppercase bg-brand-accent/30">
                  <tr>
                    <th scope="col" className="px-4 py-3">
                      {t('cashFlowForecastMonth')}
                    </th>
                    <th scope="col" className="px-4 py-3">
                      {t('cashFlowForecastInflows')}
                    </th>
                    <th scope="col" className="px-4 py-3">
                      {t('cashFlowForecastOutflows')}
                    </th>
                    <th scope="col" className="px-4 py-3">
                      {t('cashFlowForecastNetFlow')}
                    </th>
                    <th scope="col" className="px-4 py-3">
                      {t('cashFlowForecastEndBalance')}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {(data?.monthlyForecasts || []).map((monthData) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-mono text-green-400">
                        {formatCurrency(monthData.inflows, language)}
                      </td>
                      <td className="px-4 py-3 font-mono text-red-400">
                        {formatCurrency(monthData.outflows, language)}
                      </td>
                      <td
                        className={`px-4 py-3 font-mono ${monthData.netCashFlow >= 0 ? 'text-green-400' : 'text-red-400'}`}
                      >
                        {formatCurrency(monthData.netCashFlow, language)}
                      </td>
                      <td
                        className={`px-4 py-3 font-mono font-bold ${monthData.endingBalance < 0 ? 'text-red-300' : 'text-brand-text'}`}
                      >
