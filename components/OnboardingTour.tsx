    ...
    switch (step.placement) {
      ...
      case 'left':
        return {
          ...
          left: isRtl ? targetRect.right + offset : targetRect.left - offset,
          transform: isRtl ? 'translateY(-50%)' : 'translate(-100%, -50%)',
        };
      case 'right':
        return {
          top: targetRect.top + targetRect.height / 2,
          left: isRtl ? targetRect.left - offset : targetRect.right + offset,
    ...
    <div className="fixed inset-0 z-[100]">
      ...
      <AnimatePresence>
        ...
        >
          ...
          <p className="text-sm text-brand-text/90 mb-4">
            {t(step.contentKey)}
          </p>
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              {currentStep > 0 && (
                <button
                  onClick={handlePrev}
                  className="px-3 py-1 text-sm bg-brand-accent/50 hover:bg-brand-accent rounded-md transition-colors"
                >
                  {t('tourPrev')}
                </button>
              )}
              <button
                onClick={handleNext}
                className="px-4 py-1 text-sm bg-brand-teal hover:bg-teal-500 text-white font-semibold rounded-md transition-colors"
              >
