...
import { exportToPdf, exportToCsv, exportToMarkdown, AnalysisType } from '../utils/exportUtils';
import { downloadReportPdf } from '../services/geminiService';
import type { SwotData, PestelData, MarketAnalysisData, MarketResearchData, RoadmapData, LeanCanvasData, OkrWorkflowData, BudgetGeneratorData, FinancialForecastData, CashFlowForecastData, KpiDashboardData, MilestonesData, ExpansionStrategyData, PitchDeckData, InvestorMatchingData, FundraisingRoadmapData, AnalysisPoint } from '../types';

type AnalysisData = (SwotData | PestelData | MarketAnalysisData | MarketResearchData | RoadmapData | LeanCanvasData | OkrWorkflowData | BudgetGeneratorData | FinancialForecastData | CashFlowForecastData | KpiDashboardData | MilestonesData | ExpansionStrategyData | PitchDeckData | InvestorMatchingData | FundraisingRoadmapData) & { id?: string };
...
const PdfContent: React.FC<{ data: any; type: AnalysisType; t: (key: any) => string }> = ({ data, type, t }) => {
    ...
    const categories = Object.keys(data).filter(key => Array.isArray((data as any)[key]) && (data as any)[key].length > 0);
...
const ExportControls: React.FC<ExportControlsProps> = ({ analysisData, analysisType, businessDescription, targetElementId }) => {
  ...
  const isCustomizable = ['swot', 'pestel', 'market', 'marketResearch'].includes(analysisType);
  ...
  const handleServerExport = async () => {
      ...
      try {
          ...
          await downloadReportPdf(analysisData.id, `${analysisType}_official_report`);
      } catch (e) {
  ...
  const handleToggleItem = (category: string, index: number) => {
    setSelectedItems(prev => ({
      ...prev,
      [category]: prev[category].map((isSelected, i) => (i === index ? !isSelected : isSelected)),
    }));
  };
  ...
  const generatePdfFromSelection = async () => {
      ...
      Object.assign(tempContainer.style, {
          ...
          backgroundColor: '#0D1B2A', color: '#E0E1DD',
      ...
      Object.keys(selectedItems).forEach(category => {
          ...
          filteredData[category] = originalPoints.filter((_: any, index: number) => selectedItems[category][index]);
      });

      const root = ReactDOM.createRoot(tempContainer);
      // Using a simplified rendering for now for the client-side custom export
      root.render(React.createElement(PdfContent, { data: filteredData, type: analysisType, t }));
  ...
  const menuVariants = {
    ...
    visible: { opacity: 1, y: 0, scale: 1, transition: { staggerChildren: 0.07, delayChildren: 0.1 } },
          ...
          {isOpen && (
            ...
            >
              ...
              <motion.button
                ...
                className="flex items-center justify-center w-full px-4 py-2 bg-brand-secondary hover:bg-brand-accent text-sm font-semibold text-brand-text rounded-lg shadow-lg transition-colors"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h7" /></svg>
                {t('exportAsCsv')}
              </motion.button>
            </motion.div>
          )}
        ...
        {isModalOpen && (
          ...
          >
             ...
             <div className="bg-brand-secondary p-4 rounded">
                 ...
                 <button onClick={generatePdfFromSelection} className="mt-2 bg-brand-teal px-4 py-2 rounded text-white">Generate</button>
             </div>
          </motion.div>
        )}
      </AnimatePresence>
