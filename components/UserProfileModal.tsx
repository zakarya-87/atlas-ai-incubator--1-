...
import { createCheckoutSession, createPortalSession, getSubscriptionStatus } from '../services/subscriptionService';
        ...
        >
          ...
          >
            <div className="p-6 border-b border-brand-accent flex justify-between items-center">
              <h3 className="text-xl font-bold text-brand-text">Account Settings</h3>
              <button onClick={onClose} className="text-brand-light hover:text-white">&times;</button>
            </div>
              ...
              {activeTab === 'billing' ? (
                  <div className="space-y-6">
                      <div className="bg-brand-primary/30 p-4 rounded-lg border border-brand-accent/30">
                          ...
                          <div className="flex justify-between items-end">
                             <h4 className="text-2xl font-bold text-brand-text capitalize">{subStatus.plan}</h4>
                             <span className={`px-2 py-1 text-xs font-bold rounded-full ${subStatus.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                      ...
                      {subStatus.plan === 'basic' ? (
                          <div className="space-y-3">
                              <h4 className="font-bold text-brand-text">Upgrade to Pro</h4>
                ...
                <form onSubmit={handleUpdate} className="space-y-4">
                    {activeTab === 'details' && (
                        <>
                            <div>
                            <label className="block text-xs font-bold text-brand-light uppercase mb-1">Email Address</label>
                            <input
                                type="email"
                                value={email}
                                disabled
                            ...
                            />
                            <p className="text-[10px] text-brand-light/50 mt-1">Email cannot be changed.</p>
                            </div>
                            <div>
                            <label className="block text-xs font-bold text-brand-light uppercase mb-1">Full Name</label>
                            <input
                                type="text"
                                value={fullName}
                    ...
                    {activeTab === 'security' && (
                        <div>
                            <label className="block text-xs font-bold text-brand-light uppercase mb-1">New Password</label>
                            <input
                                type="password"
                                value={password}
                                ...
                                placeholder="••••••••"
                            />
                            <p className="text-[10px] text-brand-light/50 mt-1">Leave blank to keep current password.</p>
                        </div>
                    )}
