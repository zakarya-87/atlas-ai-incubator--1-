  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-brand-secondary to-brand-primary p-8 rounded-2xl border border-brand-accent/30 shadow-2xl"
      >
        <h1 className="text-3xl md:text-4xl font-bold text-brand-text">
          {t('dashboardWelcome')}{' '}
          <span className="text-brand-teal">{userName}</span>
        </h1>
        ...
        <div className="mt-6 flex items-center gap-4">
          <div className="px-4 py-2 bg-black/30 rounded-lg border border-white/10 flex items-center gap-2">
            ...
            <span
              className={`font-bold ${isPro ? 'text-brand-teal' : credits !== null && credits < 2 ? 'text-red-400' : 'text-brand-text'}`}
            >
      ...
      <motion.div variants={itemVariants}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <button
            onClick={() => onNavigate('fundamentals', 'ideaValidation')}
            className="p-4 bg-brand-secondary/40 hover:bg-brand-secondary border border-brand-accent/30 rounded-xl transition-all group text-left"
          >
            ...
            <span className="font-semibold text-brand-text block">
              {t('dashboardActionIdea')}
            </span>
          </button>

          <button
            onClick={() => onNavigate('strategy', 'swot')}
            className="p-4 bg-brand-secondary/40 hover:bg-brand-secondary border border-brand-accent/30 rounded-xl transition-all group text-left"
          >
            ...
            <span className="font-semibold text-brand-text block">
              {t('dashboardActionStrategy')}
            </span>
          </button>

          <button
            onClick={() => onNavigate('finance', 'financialForecast')}
            className="p-4 bg-brand-secondary/40 hover:bg-brand-secondary border border-brand-accent/30 rounded-xl transition-all group text-left"
          >
            ...
            <span className="font-semibold text-brand-text block">
              {t('dashboardActionFinance')}
            </span>
          </button>

          <button
            onClick={() => onNavigate('funding', 'pitchDeckGenerator')}
            className="p-4 bg-brand-secondary/40 hover:bg-brand-secondary border border-brand-accent/30 rounded-xl transition-all group text-left"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div
          variants={itemVariants}
          className="lg:col-span-2 bg-brand-secondary/20 rounded-xl p-6 border border-brand-accent/30"
        >
          ...
          <div className="space-y-3">
            {isLoading ? (
              ...
              history.map((record) => (
                <div
                  key={record.id}
                  className="p-3 rounded-lg bg-brand-secondary/30 flex items-center justify-between group hover:bg-brand-secondary/50 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-brand-teal"></div>
                    <div>
                      <p className="text-sm font-bold text-brand-text group-hover:text-white transition-colors">
                  ...
                  <button
                    onClick={() => onNavigate('audit', 'trail')}
                    className="text-xs px-2 py-1 bg-brand-primary/50 rounded text-brand-light hover:text-white"
                  >
                    View
                  </button>
                </div>
              ...
              <p className="text-brand-light/50 text-center py-8">
        ...
        <motion.div
          variants={itemVariants}
          className="bg-brand-secondary/20 rounded-xl p-6 border border-brand-accent/30"
        >
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-bold text-brand-text">
              {t('dashboardIntegrations')}
            </h3>
            <button
              onClick={() => onNavigate('integrations', 'connectors')}
              className="text-sm text-brand-teal hover:text-white transition-colors"
            >
              Manage
            </button>
          ...
          <div className="space-y-4">
            {['ga', 'meta'].map((provider) => {
              const isConnected =
                ...
                'connected';
              const label =
                provider === 'ga' ? t('integrationsGA') : t('integrationsMeta');

              return (
                <div
                  key={provider}
                  className="flex items-center justify-between p-3 bg-brand-secondary/30 rounded-lg"
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center ${isConnected ? 'bg-green-500/20 text-green-400' : 'bg-brand-primary/50 text-brand-light'}`}
                    >
                      {provider === 'ga' ? (
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                        ...
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2V7a2 2 0 012-2h4a2 2 0 002-2V4a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l4 4h5a2 2 0 002-2v-6a2 2 0 00-2-2z"
                          />
                        </svg>
                      )}
                    </div>
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${isConnected ? 'bg-green-500/20 text-green-400' : 'bg-brand-primary/50 text-brand-light'}`}
                  >
                    {isConnected
                      ? t('dashboardConnected')
                      : t('dashboardDisconnected')}
                  </span>
                </div>
              );
            })}
