...
import { FundingTool } from '../types';

interface FundingSubNavProps {
  activeTool: FundingTool;
...
const FundingSubNav: React.FC<FundingSubNavProps> = ({ activeTool, onToolChange }) => {
  ...
  const navItems: { key: FundingTool; label: string }[] = [
    { key: 'pitchDeckGenerator', label: t('fundingNavPitchDeckGenerator') },
    { key: 'investorDatabase', label: t('fundingNavInvestorDatabase') },
    { key: 'fundraisingRoadmap', label: t('fundingNavFundraisingRoadmap') },
  ];
