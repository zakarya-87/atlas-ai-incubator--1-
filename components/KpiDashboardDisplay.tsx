...
const KpiCard: React.FC<{ kpi: Kpi }> = ({ kpi }) => {
  ...
  if (
    ...
    insightLower.includes('high') ||
    insightLower.includes('low') ||
    insightLower.includes('warning') ||
    insightLower.includes('optimize')
  ) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-start mt-3 pt-3 border-t border-brand-accent/30 ${insightColorClass}`}
      >
        <div className="flex-shrink-0 mr-2 rtl:mr-0 rtl:ml-2 mt-0.5">
          {icon}
        </div>
...
const KpiDashboardDisplay: React.FC<{ data: KpiDashboardData }> = ({
  data,
}) => {
  const { t } = useLanguage();
    ...
    return (
      ...
      >
        <div className="text-brand-text/60">
          <p>No KPI dashboard data available.</p>
    ...
    >
      ...
      >
        <h4 className="text-xl font-bold text-brand-light mb-2">
          {t('kpiDashboardExecutiveSummary')}
        </h4>
