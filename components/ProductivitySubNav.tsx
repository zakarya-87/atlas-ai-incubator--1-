...
interface ProductivitySubNavProps {
  activeTool: ProductivityTool;
...
const ProductivitySubNav: React.FC<ProductivitySubNavProps> = React.memo(({
  activeTool,
  onToolChange,
}) => {
  const { t } = useLanguage();
  ...
  const navItems: { key: ProductivityTool; label: string }[] = [
    { key: 'taskManager', label: t('productivityNavTaskManager') },
  ];
...
export default ProductivitySubNav;
