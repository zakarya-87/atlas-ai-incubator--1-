...
const AuditTrailDisplay: React.FC<AuditTrailDisplayProps> = ({ history, onView, onDelete, modules }) => {
  const { t } = useLanguage();
  ...
  if (history.length === 0) {
    return (
      <div className="text-center p-8 text-brand-light max-w-lg mx-auto">
        <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-16 w-16 text-brand-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
        </svg>
        <h2 className="mt-6 text-2xl font-bold text-brand-text">{t('auditTrailTitle')}</h2>
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-brand-text">{t('auditTrailTitle')}</h2>
          ...
          {filteredHistory.map(record => (
            ...
            >
              <div className="flex-grow">
                <p className="font-bold text-brand-text">{t(record.toolNameKey as any)}</p>
